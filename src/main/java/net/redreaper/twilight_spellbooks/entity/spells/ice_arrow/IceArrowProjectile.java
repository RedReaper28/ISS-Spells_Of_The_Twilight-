package net.redreaper.twilight_spellbooks.entity.spells.ice_arrow;
...
public class IceArrowProjectile extends AbstractMagicProjectile {
    private static final EntityDataAccessor<Boolean> IN_GROUND = SynchedEntityData.defineId(IceArrowProjectile.class, EntityDataSerializers.BOOLEAN);
    public IceArrowProjectile(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
        super.setNoGravity(true); // avoid lcoal gravity flag by using super
    }

    public IceArrowProjectile(Level pLevel, LivingEntity pShooter) {
        this(ModEntities.ICE_ARROW.get(), pLevel);
    ...
    protected void onHitBlock(BlockHitResult pResult) {
        ...
        if (!level().isClientSide && !hasEmittedPoison) {
            createFrostField(pResult.getLocation());
        }
    }
    ...
    protected void onHitEntity(@NotNull EntityHitResult entityHitResult) {
        ...
        if (hit) {
            if (!ignore) {
                if (!level().isClientSide && !hasEmittedPoison)
                    createFrostField(entity.position());
    ...
    public void impactParticles(double x, double y, double z) {
        MagicManager.spawnParticles(level(), ParticleHelper.SNOW_DUST, x, y, z, 15, .03, .03, .03, 0.2, true);
    }
