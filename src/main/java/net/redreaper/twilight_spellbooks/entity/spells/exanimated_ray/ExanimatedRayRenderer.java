...
public class ExanimatedRayRenderer extends EntityRenderer<ExanimatedRayVisualEntity> {

    public static final ModelLayerLocation MODEL_LAYER_LOCATION = new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(TwilightSpellbooks.MOD_ID, "exanimated_ray_model"), "main");
    private static final ResourceLocation TEXTURE_CORE = TwilightSpellbooks.id("textures/entity/exanimated_ray/core.png");
    private static final ResourceLocation TEXTURE_OVERLAY = TwilightSpellbooks.id("textures/entity/exanimated_ray/overlay.png");

    private final ModelPart body;

    public ExanimatedRayRenderer(EntityRendererProvider.Context context) {
        super(context);
    ...
    @Override
    public boolean shouldRender(ExanimatedRayVisualEntity pLivingEntity, Frustum pCamera, double pCamX, double pCamY, double pCamZ) {
        return true;
    }

    @Override
    public void render(ExanimatedRayVisualEntity entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int light) {
        poseStack.pushPose();
        float lifetime = ExanimatedRayVisualEntity.lifetime;
        float scalar = .25f;
    ...
    @Override
    public ResourceLocation getTextureLocation(ExanimatedRayVisualEntity entity) {
        return TEXTURE_CORE;
    }
}
