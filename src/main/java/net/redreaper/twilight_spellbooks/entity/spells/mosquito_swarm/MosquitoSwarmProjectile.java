package net.redreaper.twilight_spellbooks.entity.spells.mosquito_swarm;
...
public class MosquitoSwarmProjectile extends PathfinderMob implements AntiMagicSusceptible/*, NoKnockbackProjectile*/ {

    public MosquitoSwarmProjectile(EntityType<? extends PathfinderMob> pEntityType, Level pLevel) {
    ...
    public MosquitoSwarmProjectile(Level level, @Nullable Entity owner, @Nullable Entity target, float damage) {
        this(ModEntities.MOSQUITO_SWARM.get(), level);
        setOwner(owner);
        setTarget(target);
        this.damage = damage;
    }
    ...
    public void tick() {
        if (level().isClientSide) {
            for (int i = 0; i < 2; i++) {
                ...
                level().addParticle(ModParticleHelper.MOSQUITO, getX() + spawn.x, getY() + this.getBbHeight() * .5f + spawn.z, getZ() + spawn.z, motion.x, motion.y, motion.z);
            }
        }
        super.tick();
    ...
    protected void customServerAiStep() {
        ...
        if ((this.tickCount % 7) == 0) {
            ...
            this.playSound(TFSounds.MOSQUITO.get(), .25f * fade, .95f + Utils.random.nextFloat() * .1f);
        }
            ...
            this.level().getEntities(this, this.getBoundingBox().inflate(inflate), this::canHitEntity).forEach(
                    (entity) -> {
                        if (canHitEntity(entity)) {
                            boolean hit = DamageSources.applyDamage(entity, damage, ModSpells.MOSQUITO_SWARM.get().getDamageSource(this, getOwner()));
                            if (hit) {
