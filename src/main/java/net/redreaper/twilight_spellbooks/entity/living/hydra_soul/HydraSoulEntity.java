package net.redreaper.twilight_spellbooks.entity.living.hydra_soul;
...
public class HydraSoulEntity extends Entity implements AntiMagicSusceptible {
    private static final EntityDataAccessor<Vector3f> DATA_RESPAWN_POS = SynchedEntityData.defineId(HydraSoulEntity.class, EntityDataSerializers.VECTOR3);
    private static final double MOVE_PER_TICK = .12;

    public HydraSoulEntity(EntityType<?> type, Level level) {
        super(type, level);
        this.noPhysics = true;
        this.setNoGravity(true);
    }

    public HydraSoulEntity(Level level, Vec3 start, Vec3 respawnPos) {
        this(ModEntities.HYDRA_SOUL.get(), level);
        this.setPos(start);
    ...
    public @NotNull InteractionResult interactAt(@NotNull Player player, @NotNull Vec3 vec, @NotNull InteractionHand hand) {
        ...
        if (isAtSpawn() && itemStack.is(ModItems.REGENERATING_CHOP)) {
            if (player.level() instanceof ServerLevel serverLevel) {
                ...
                this.playSound(TFSounds.HYDRA_GROWL.get(), 2, 0.75f);
                MagicManager.spawnParticles(level(), ModParticleHelper.HYDRA_FIRE_PARTICLE, getX(), getY(), getZ(), 50, .1, .1, .1, 0.3, false);


                float f = player.getYRot() + 180;

                Hydra myCreature = new Hydra(TFEntities.HYDRA.get(), serverLevel);
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                level().addParticle(
                        ModParticleHelper.HYDRA_FIRE_PARTICLE,
                        true,
                        getX() + offset.x,
