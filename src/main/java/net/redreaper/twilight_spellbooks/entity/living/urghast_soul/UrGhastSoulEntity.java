package net.redreaper.twilight_spellbooks.entity.living.urghast_soul;
...
public class UrGhastSoulEntity extends Entity implements AntiMagicSusceptible {
    private static final EntityDataAccessor<Vector3f> DATA_RESPAWN_POS = SynchedEntityData.defineId(UrGhastSoulEntity.class, EntityDataSerializers.VECTOR3);
    private static final double MOVE_PER_TICK = .12;

    public UrGhastSoulEntity(EntityType<?> type, Level level) {
        super(type, level);
        this.noPhysics = true;
        this.setNoGravity(true);
    }

    public UrGhastSoulEntity(Level level, Vec3 start, Vec3 respawnPos) {
        this(ModEntities.URGHAST_SOUL.get(), level);
        this.setPos(start);
    ...
    public @NotNull InteractionResult interactAt(@NotNull Player player, @NotNull Vec3 vec, @NotNull InteractionHand hand) {
        ...
        if (isAtSpawn() && itemStack.is(ModItems.CARMINITE_HEART)) {
            if (player.level() instanceof ServerLevel serverLevel) {
                ...
                this.playSound(SoundEvents.GHAST_HURT, 2, 0.75f);
                MagicManager.spawnParticles(level(), TFParticleType.GHAST_TRAP.get(), getX(), getY(), getZ(), 50, .1, .1, .1, 0.3, false);


                float f = player.getYRot() + 180;

                UrGhast myCreature = new UrGhast(TFEntities.UR_GHAST.get(), serverLevel);
