package net.redreaper.twilight_spellbooks.entity.living.snow_queen_soul;
...
public class SnowQueenSoulEntity extends Entity implements AntiMagicSusceptible {
    private static final EntityDataAccessor<Vector3f> DATA_RESPAWN_POS = SynchedEntityData.defineId(SnowQueenSoulEntity.class, EntityDataSerializers.VECTOR3);
    private static final double MOVE_PER_TICK = .12;

    public SnowQueenSoulEntity(EntityType<?> type, Level level) {
        super(type, level);
        this.noPhysics = true;
        this.setNoGravity(true);
    }

    public SnowQueenSoulEntity(Level level, Vec3 start, Vec3 respawnPos) {
        this(ModEntities.SNOW_QUEEN_SOUL.get(), level);
        this.setPos(start);
    ...
    public @NotNull InteractionResult interactAt(@NotNull Player player, @NotNull Vec3 vec, @NotNull InteractionHand hand) {
        ...
        if (isAtSpawn() && itemStack.is(ModTags.DEAD_KING_RESPAWNABLE)) {
            if (player.level() instanceof ServerLevel serverLevel) {
                ...
                MagicManager.spawnParticles(level(), TFParticleType.SNOW_GUARDIAN.get(), getX(), getY(), getZ(), 50, .1, .1, .1, 0.3, false);


                float f = player.getYRot() + 180;

                SnowQueen myCreature = new SnowQueen(TFEntities.SNOW_QUEEN.get(), serverLevel);
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                level().addParticle(
                        TFParticleType.SNOW_GUARDIAN.get(),
                        true,
                        getX() + offset.x,
