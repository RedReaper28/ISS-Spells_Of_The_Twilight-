...
public class HydraMortarShotSpell extends AbstractSpell {
        ...
        if (!world.isClientSide) {
            ...
            float explosionRadius = 1.0F + (float)spellLevel * 0.25F;
                ...
                for (int angle : angles) {
                    ...
                    dagger.setAoeDamage(getAOEDamage(spellLevel, entity));
    ...
    public float getDamage(int spellLevel, LivingEntity caster) {
        if (caster == null) {
            ...
            double maturePower = caster.getAttributeValue(AttributeRegistry.NATURE_SPELL_POWER);
            if (firePower == maturePower) {
                return (float)((double)5+ 5  * ((double)1.5F * firePower + (double)1.5F * maturePower));
            }
            else {
                return (float)((double)5+ 5  * (firePower + maturePower));
            }
        }
    }
