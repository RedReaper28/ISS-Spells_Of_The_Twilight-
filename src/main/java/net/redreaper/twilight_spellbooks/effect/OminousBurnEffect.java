...
@EventBusSubscriber
public class OminousBurnEffect extends MagicMobEffect implements ISyncedMobEffect {
    public OminousBurnEffect(MobEffectCategory pCategory, int pColor) {
    ...
    public boolean applyEffectTick(LivingEntity p_296276_, int p_296233_) {
        ...
        Holder.Reference<DamageType> dType = dTypeReg.getHolder(NeoForgeMod.POISON_DAMAGE).orElse(dTypeReg.getHolderOrThrow(TFDamageTypes.OMINOUS_FIRE));
        p_296276_.hurt(new DamageSource(dType), 1);
        return true;
    }

    public void clientTick(LivingEntity livingEntity, MobEffectInstance instance) {
        ParticleOptions particle = TFParticleType.OMINOUS_FLAME.get();
